...
RSpec.describe VisitorPattern do
  ...
  describe VisitorPattern::ShoppingCart do
    context "when visiting an aisle" do
      ...
      it "returns nil" do
        expect(empty_cart.visit_aisle aisle).to be_nil
      end
    end
    ...
    context "when visiting an empty shelf" do
      it "raises the shelf's error" do
        expect { empty_cart.visit_shelf empty_shelf }.to raise_error RuntimeError
      end
    end
  end
  ...
  describe VisitorPattern::Shelf do
    ...
    context "when empty" do
      ...
      context "and items are added" do
        ...
        it "returns to empty after the only item is popped" do
          item = spy()
          empty_shelf.push item

          expect(empty_shelf.pop).to equal item
          expect(empty_shelf.size).to eql 0
          expect(empty_shelf.face).to equal nil
        end
      end
